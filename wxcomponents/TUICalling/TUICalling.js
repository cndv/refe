...
Component({
    ...
    methods: {
        ...
        handleNetStatus(data) {
            ...
            data.playerList.map((item) => {
                if (item.netStatus.netQualityLevel > 4) {
                    const name =
                        data.playerList.length > 1 ? item.nick : "对方";
                    wx.showToast({
                        icon: "none",
                        title: `${name}当前网络不佳`,
                    });
        ...
        handleInviteeReject(event) {
            console.log(
                `${TAG_NAME}, handleInviteeReject, event${JSON.stringify(
                    event
                )}`
            );
            wx.showToast({
        ...
        handleLineBusy(event) {
            console.log(
                `${TAG_NAME}, handleLineBusy, event${JSON.stringify(event)}`
            );
            wx.showToast({
        ...
        handleCallingCancel(event) {
            console.log(
                `${TAG_NAME}, handleCallingCancel, event${JSON.stringify(
                    event
                )}`
            );
            wx.showToast({
        ...
        handleCallingTimeout(event) {
            console.log(
                `${TAG_NAME}, handleCallingTimeout, event${JSON.stringify(
                    event
                )}`
            );
            wx.showToast({
        ...
        handleCallingUser(userIDList) {
            ...
            console.log(
                `${TAG_NAME}, handleCallingUser, userProfile`,
                userProfile
            );
            let nick = "";
        ...
        async init() {
            console.log(
                "初始化:TRTCCalling init",
                this.data.config.userID,
                this.data.config.userSig
            );
            try {
                log("初始化:TRTCCalling init");
