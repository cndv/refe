<template>
  <uni-shadow-root class="TUICalling-TUICalling"
    ><view :class="'TUICalling ' + (callStatus === 'idle' ? 'hidden' : 'show')">
      <view class="TRTCCaling-container">
        <TUI-Calling
          ...
          @callingEvent="handleCallingEvent"
        ></TUI-Calling>
        <TUI-Connected
          v-if="callStatus === 'connection'"
          :playerList="playerList"
          :pusher="pusher"
          :callType="config.type"
          :soundMode="soundMode"
          :screen="screen"
          @connectedEvent="handleConnectedEvent"
        ></TUI-Connected>
      </view> </view
  ></uni-shadow-root>
</template>
...
<script>
import TUICalling from "./component/calling/calling.vue";
import TUIConnected from "./component/connected/connected.vue";
global["__wxVueOptions"] = {
  components: { "TUI-Calling": TUICalling, "TUI-Connected": TUIConnected },
};

global["__wxRoute"] = "TUICalling/TUICalling";
...
Component({
  ...
  methods: {
    ...
    handleNewInvitationReceived(event) {
      this.initCall();
      console.log(
        `${TAG_NAME}, handleNewInvitationReceived, event${JSON.stringify(
          event
        )}`
      );
    ...
    handleUserAccept(event) {
      console.log(
        `${TAG_NAME}, handleUserAccept, event${JSON.stringify(event)}`
      );
      this.setData({
    ...
    handleInviteeReject(event) {
      console.log(
        `${TAG_NAME}, handleInviteeReject, event${JSON.stringify(event)}`
      );
      wx.showToast({
    ...
    handleNoResponse(event) {
      console.log(
        `${TAG_NAME}, handleNoResponse, event${JSON.stringify(event)}`
      );
      wx.showToast({
    ...
    handleCallingCancel(event) {
      console.log(
        `${TAG_NAME}, handleCallingCancel, event${JSON.stringify(event)}`
      );
      wx.showToast({
    ...
    handleCallingTimeout(event) {
      console.log(
        `${TAG_NAME}, handleCallingTimeout, event${JSON.stringify(event)}`
      );
      wx.showToast({
        title: `${this.handleCallingUser(
          event.data.timeoutUserList
        )}超时无应答`,
      });
    },
    handleCallingUser(userIDList) {
      const remoteUsers = [...this.data.remoteUsers];
      const userProfile = remoteUsers.filter((item) =>
        userIDList.some((userItem) => `${userItem}` === item.userID)
      );
      this.setData({
        remoteUsers: remoteUsers.filter((item) =>
          userIDList.some((userItem) => userItem !== item.userID)
        ),
      });
    ...
    _addTSignalingEvent() {
      // 被邀请通话
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.INVITED,
        this.handleNewInvitationReceived,
        this
      );
      // 用户接听
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.USER_ACCEPT,
        this.handleUserAccept,
        this
      );
      // 用户进入通话
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.USER_ENTER,
        this.handleUserEnter,
        this
      );
      // 用户离开通话
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.USER_LEAVE,
        this.handleUserLeave,
        this
      );
      // 用户离开通话
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.USER_UPDATE,
        this.handleUserUpdate,
        this
      );
      // 用户拒绝通话
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.REJECT,
        this.handleInviteeReject,
        this
      );
      // 用户无响应
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.NO_RESP,
        this.handleNoResponse,
        this
      );
      // 用户忙线
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.LINE_BUSY,
        this.handleLineBusy,
        this
      );
      // 通话被取消
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.CALLING_CANCEL,
        this.handleCallingCancel,
        this
      );
      // 通话超时未应答
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.CALLING_TIMEOUT,
        this.handleCallingTimeout,
        this
      );
      // 通话结束
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.CALL_END,
        this.handleCallingEnd,
        this
      );
      // SDK Ready 回调
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.SDK_READY,
        this.handleSDKReady,
        this
      );
      // 被踢下线
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.KICKED_OUT,
        this.handleKickedOut,
        this
      );
      // 切换通话模式
      wx.$TRTCCalling.on(
        wx.$TRTCCalling.EVENT.CALL_MODE,
        this.handleCallMode,
        this
      );
    },
    ...
    async call(params) {
      this.initCall();
      wx.$TRTCCalling
        .call({ userID: params.userID, type: params.type })
        .then((res) => {
    ...
    async init() {
      try {
        ...
        throw new Error("TRTCCalling login failure123", error);
      }
    },
    // 销毁 TRTCCalling
    destroyed() {
...
export default global["__wxComponents"]["TUICalling/TUICalling"];
</script>
